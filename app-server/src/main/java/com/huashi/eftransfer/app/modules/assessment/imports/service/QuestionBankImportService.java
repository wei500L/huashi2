...
public class QuestionBankImportService {
    ...
    public byte[] downloadTemplate(boolean json) {
        try {
            if (json) {
                ...
                template.put("Questionnaire", Map.of("code", "LEXIBRIDGE_RESEARCH_V1", "title", "", "description", "", "durationMinutes", 60, "scoringVersion", "SCORING_V1", "aiPromptVersion", "assessment-analysis/v2"));
            ...
            try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream output = new ByteArrayOutputStream()) {
                writeSheet(workbook, "Questionnaire", List.of("code", "title", "description", "durationMinutes", "scoringVersion", "aiPromptVersion"), List.of("LEXIBRIDGE_RESEARCH_V1", "", "", "60", "SCORING_V1", "assessment-analysis/v2"));
    ...
    private QuestionBankImportPackageRequest parse(String fileName, byte[] bytes) throws IOException {
        ...
        try (InputStream input = new java.io.ByteArrayInputStream(bytes); Workbook workbook = new XSSFWorkbook(input)) {
            ...
            for (int i=1; i<=itemsSheet.getLastRowNum(); i++) { Row row=itemsSheet.getRow(i); if(row==null||cell(formatter,row,0).isBlank()) continue; items.add(new QuestionBankImportPackageRequest.ItemRow(cell(formatter,row,0),cell(formatter,row,1),cell(formatter,row,2),cell(formatter,row,3),cell(formatter,row,4),split(cell(formatter,row,5)),cell(formatter,row,6),bool(formatter,row,7),bool(formatter,row,8),decimal(formatter,row,9),cell(formatter,row,10),cell(formatter,row,11),cell(formatter,row,12),cell(formatter,row,13),cell(formatter,row,14),integer(formatter,row,15),integer(formatter,row,16),blankToNull(cell(formatter,row,17)),integer(formatter,row,18),integer(formatter,row,19),bool(formatter,row,20),blankToNull(cell(formatter,row,21)),blankToNull(cell(formatter,row,22)))); }
            List<QuestionBankImportPackageRequest.OptionRow> options = new ArrayList<>();
            for (int i=1; i<=optionsSheet.getLastRowNum(); i++) { Row row=optionsSheet.getRow(i); if(row==null||cell(formatter,row,0).isBlank()) continue; options.add(new QuestionBankImportPackageRequest.OptionRow(cell(formatter,row,0),cell(formatter,row,1),cell(formatter,row,2),bool(formatter,row,3),cell(formatter,row,4),blankToNull(cell(formatter,row,5)))); }
