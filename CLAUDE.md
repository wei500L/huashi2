...
| 2026-08-12 | 研究问卷数据闭环 / 附件 / 群体 AI 报告 | 教师端 `/teacher/research?tab=data` 接入发布选择、漏斗口径、题目/维度统计与匿名答卷列表；新增单份答卷页与群体报告页。公开答卷按 `participant_id` 授权，不再依赖班级/学生账号。新增 `FILE_UPLOAD` 题型、对象存储与扫描后鉴权下载。群体报告基于不可变统计快照，最小样本 5，失败规则降级且不标成模型结论。普通导出脱敏，不暴露 objectKey/参与码/IP/敏感资料。 |
