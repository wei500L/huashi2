...
import { formatDuration, formatScore } from '../formatters';
...
export const ResearchAttemptList: React.FC<{
...
}> = ({ rows, loading, error, onRetry, onOpen }) => {
  const columns: Array<DataTableColumn<ResearchAttemptSummaryVO>> = [
    ...
    { id: 'score', header: '得分', render: (row) => formatScore(row.percentageScore) },
    { id: 'duration', header: '用时', render: (row) => formatDuration(row.effectiveDurationMs) },
    { id: 'quality', header: '质量', render: (row) => row.qualityFlags.length ? row.qualityFlags.join('、') : '正常' },
    { id: 'attachments', header: '附件', render: (row) => String(row.attachmentCount) },
    { id: 'ai', header: 'AI', render: (row) => row.aiAnalysisStatus || '—' },
        ...
        {rows.map((row) => (
          ...
          >
            ...
            <p className="mt-2 text-sm text-slate-500">
              {row.submittedAt ? formatDateTime(row.submittedAt) : '尚未提交'} · 得分 {formatScore(row.percentageScore)} · 用时 {formatDuration(row.effectiveDurationMs)}
            </p>
          </button>
        ))}
      </div>
