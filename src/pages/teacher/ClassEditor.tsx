...
const TeacherClassEditorPage: React.FC = () => {
  ...
  return (
    <div className="page-stack pb-16">
      ...
      <form ref={formRef} className="page-stack" onSubmit={handleSubmit}>
        ...
        {feedback && (
          <div className="min-w-0 rounded-2xl border border-emerald-500/20 bg-emerald-500/10 p-4 text-emerald-700 sm:p-6 dark:text-emerald-300">
            {feedback}
          </div>
        )}
        ...
        <section className="min-w-0 rounded-2xl liquid-glass-panel p-4 sm:rounded-[2.5rem] sm:p-6 md:p-8">
          ...
          <div className="grid min-w-0 grid-cols-1 gap-6 md:grid-cols-2">
            <label className="min-w-0 space-y-3">
              <div className="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-3">
                ...
                <button
                  ...
                  className="inline-flex w-full items-center justify-center gap-2 rounded-full border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-600 transition hover:border-primary/40 hover:text-primary disabled:cursor-not-allowed disabled:opacity-60 sm:w-auto dark:border-white/10 dark:text-white/70"
                >
              ...
              <input
                ...
                className="w-full min-w-0 rounded-[1.4rem] border border-slate-200 bg-white/70 px-4 py-3 text-sm outline-none transition focus:border-primary dark:border-white/10 dark:bg-white/5"
              />
            ...
            <label className="min-w-0 space-y-3 md:col-span-2">
        ...
        <section className="min-w-0 rounded-2xl border border-slate-200/70 bg-white/60 p-4 sm:p-6 dark:border-white/10 dark:bg-white/[0.03]">
          <div className="page-toolbar">
            <div className="min-w-0 text-sm text-slate-500 dark:text-white/45">
              保存后会直接进入班级详情页，继续分配学生或发布测评。
            </div>
            <button
              type="submit"
              disabled={!canSubmit || mutation.isPending}
              className="btn-liquid inline-flex w-full items-center justify-center gap-2 px-5 py-3 text-white disabled:cursor-not-allowed disabled:opacity-60 sm:w-auto"
            >
              <Save size={16} />
              {mutation.isPending ? '保存中…' : '保存班级'}
            </button>
