...
const DashboardPage: React.FC = () => {
  ...
  return (
    <div className="space-y-10 pb-20">
      ...
      <section className="surface-panel relative min-w-0 overflow-hidden rounded-2xl border border-primary/15 bg-[radial-gradient(circle_at_80%_10%,rgba(45,212,191,0.16),transparent_38%),linear-gradient(135deg,rgba(15,118,110,0.08),transparent_65%)] p-4 shadow-[0_24px_70px_rgba(15,118,110,0.08)] sm:rounded-[2.5rem] sm:p-6 md:p-8">
        <div className="relative z-10 grid min-w-0 gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(0,22rem)] xl:items-center xl:gap-8">
          <div className="min-w-0">
            ...
            <h1 className="mt-4 max-w-3xl text-2xl font-semibold tracking-tight text-slate-950 dark:text-white sm:text-3xl md:text-5xl">{overview?.studentName ? t('dashboard.hero.greeting', { name: overview.studentName }) : t('dashboard.hero.firstUseTitle')}</h1>
          ...
          <div className="rounded-[2rem] border border-primary/15 bg-surface/65 p-5 backdrop-blur-sm" aria-label={t('dashboard.map.label')}><div className="flex items-center justify-between gap-3"><SectionEyebrow>{t('dashboard.map.title')}</SectionEyebrow><MapPinned size={17} className="text-primary" aria-hidden="true" /></div><div className="mt-7 flex items-center gap-2"><div className="flex h-16 w-16 shrink-0 flex-col items-center justify-center rounded-2xl border border-primary/20 bg-primary/10 text-primary"><span className="text-[10px] font-bold uppercase tracking-[0.18em]">EN</span></div><div className="relative h-px flex-1 bg-primary/30"><span className="absolute left-1/2 top-1/2 flex h-7 w-7 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border border-primary/20 bg-surface text-primary"><ArrowRight size={13} aria-hidden="true" /></span></div><div className="flex h-16 w-16 shrink-0 flex-col items-center justify-center rounded-2xl border border-sky-500/20 bg-sky-500/10 text-sky-700 dark:text-sky-200"><span className="text-[10px] font-bold uppercase tracking-[0.18em]">FR</span><span className="mt-1 text-xs font-semibold">{overview?.frenchLevel || '--'}</span></div></div><p className="mt-5 text-xs leading-5 text-slate-500 dark:text-white/45">{t('dashboard.map.description')}</p></div>
        </div>
      </section>
      ...
      <section className="hidden liquid-glass-panel rounded-2xl p-4 edge-light sm:rounded-3xl sm:p-6 md:p-8">
        <div className="flex flex-col items-start justify-between gap-8 lg:flex-row">
          ...
          <div className="grid w-full min-w-0 gap-4 sm:grid-cols-2">
            <div className="rounded-[2rem] border border-slate-200/80 bg-white/60 p-5 dark:border-white/10 dark:bg-white/5">
              <SectionEyebrow className="text-xs">{t('ui.fields.frenchLevel')}</SectionEyebrow>
              <div className="mt-3 text-2xl font-black text-slate-900 dark:text-white">
                {overview?.frenchLevel || '--'}
              </div>
            </div>
            <div className="rounded-[2rem] border border-slate-200/80 bg-white/60 p-5 dark:border-white/10 dark:bg-white/5">
