      ...
      <section className="research-entry-grid min-w-0" aria-label="参与研究">
        ...
        <div className="research-access-card min-w-0">
          ...
          <form onSubmit={onVerify} className="research-code-form min-w-0">
            ...
            <button type="submit" disabled={verifying || !participationCode.trim()} className="research-primary-button w-full sm:w-auto">{verifying ? '正在验证…' : '验证并开始'}<ArrowUpRight size={18} className="shrink-0" /></button>
          </form>
  ...
  <main className="research-result min-w-0">
    <div className="research-result-inner min-w-0">
      ...
      <div className="research-analysis min-w-0">
        ...
        {result.aiAnalysis ? <p className="break-words">{result.aiAnalysis.performanceOverview}</p> : <p>分析状态：{result.aiAnalysisStatus || 'PENDING'}。稍后重新打开页面即可查看更新。</p>}
      </div>
    </div>
  </main>
);
...
function hasPublicSessionMarker(releaseCode: string): boolean {
  const raw = window.localStorage.getItem(publicSessionMarkerKey(releaseCode));
  const verifiedAt = raw ? Number(raw) : Number.NaN;
  if (!Number.isFinite(verifiedAt) || Date.now() - verifiedAt >= PUBLIC_SESSION_MARKER_TTL_MS) {
...
function rememberPublicSession(releaseCode: string): void {
  window.localStorage.setItem(publicSessionMarkerKey(releaseCode), String(Date.now()));
}

function forgetPublicSession(releaseCode: string): void {
  window.localStorage.removeItem(publicSessionMarkerKey(releaseCode));
}
      ...
      <div className="research-assessment-inner min-w-0">
        <header className="research-assessment-header min-w-0">
          <div className="min-w-0">
            ...
            <div className="research-progress-copy">已答 {answeredCount} / {attempt.questionCount}</div>
          </div>
          <div className="research-save-status shrink-0">
            {saving ? <Clock3 size={15} /> : <Save size={15} />}
            {saveMessage || (attempt.lastSavedAt ? '答卷已恢复' : '自动保存已开启')}
          </div>
        </header>
        <div className="research-progress">
          <span style={{ width: `${Math.round((answeredCount / Math.max(1, attempt.questionCount)) * 100)}%` }} />
        </div>
        <section className="research-question-card min-w-0">
          {currentQuestion.sectionTitle ? <div className="research-section-index">{currentQuestion.sectionTitle}</div> : null}
          ...
          <div className="research-question-number">QUESTION {String(currentQuestion.questionOrder).padStart(2, '0')}</div>
          <h1 className="break-words">{currentQuestion.stemText}</h1>
          ...
          <footer className="research-question-footer">
            <button type="button" disabled={selectedIndex === 0} onClick={() => setSelectedIndex((value) => Math.max(0, value - 1))} className="research-quiet-button">
              <ChevronLeft size={17} />上一题
            </button>
            {selectedIndex < attempt.questions.length - 1 ? (
              <button type="button" onClick={() => setSelectedIndex((value) => Math.min(attempt.questions.length - 1, value + 1))} className="research-primary-button w-full sm:w-auto">
                下一题<ChevronRight size={17} className="shrink-0" />
              </button>
            ) : (
              <button type="button" disabled={submitting} onClick={() => void submit()} className="research-primary-button w-full sm:w-auto">
