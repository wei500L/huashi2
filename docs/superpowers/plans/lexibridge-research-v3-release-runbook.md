...
| 正式题结构 | Partie 1A/1B/2/3/4 共 60 题，按语境层级（WORD/PHRASE/SENTENCE/CLOZE/READING） | 同 V1，仅拆 P1B 与加粗强调 | 4 个 section：词义单选 27 / 句子选词 1 / 判断正误 27 / 拼写 174（共 229 计分题） |
...
- 题量：T1=27，T2=1，T3=27，T4=174（共 229 计分题），V3 直接使用完整生产批准题库，不再抽样缩减。
- 全部 229 题：`LEXICAL_REVIEW_STATUS=APPROVED` + `PEDAGOGIC_REVIEW_STATUS=APPROVED`。
- 覆盖状态：`MAXIMUM_RULE_COMPLIANT_COVERAGE`；生产审计强制校验“语义批准池 ∪ 拼写合格池”全部进入题库且目标词不重复，任何漏词均使审计失败。
- 题型 3 严格执行 0811 文档，因此全部为 F；不再作为内容批准阻塞项。
...
- [x] **题型三设计决策**：严格执行 0811 文档方案 A，英语迁移义命题自然全部为 F，不添加控制题。
...
1. **备份**（必须先做，任何一步之前）：
   ...
   ```
2. **Schema 变更**：部署包含 `assessment_attempt_answer` 新增两列（`spelling_wrong_attempt_count`、`spelling_hint_shown_at`）的版本。开发期按 CLAUDE.md 约定直接更新 `schema.sql`（本次已更新；生产库需在停机窗口执行等价 ALTER）。
3. **后端部署**：部署 shared-kernel + app-server 新版本（含 SPELLING 题型、V3 种子、SCORING_V3、拼写提示接口）。
4. **V3 种子加载**：生产环境设置 `APP_ASSESSMENT_LEXIBRIDGE_V3_SEED_ENABLED=true` 启动一次，生成内容状态为 APPROVED、试卷状态为 DRAFT 的 `LEXIBRIDGE_RESEARCH_V3` 问卷。
5. **导入核验**：确认导入记录无 OPEN/REJECTED review issue，questionnaire/version 保持种子中的 APPROVED；试卷仍为 DRAFT，不自动发布。
6. **发布**：创建新 release（`LEXIBRIDGE_RESEARCH_V3`），生成参与码，**保留 V1 release 不触碰**。
7. **回滚路径**：V1 release 及其数据、种子文件、代码路径均未改动；若 V3 出问题，直接停用 V3 release、恢复备份即可回到 V1 现状。
...
- 不得把题型三改成混合真假或添加未标注的 control 题（除非用户在 TYPE3 决策文档批准）。
